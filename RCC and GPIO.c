...
int main(void)
{
  ...
  /* USER CODE BEGIN Init */
	...
	((RCC_Reg_struct *) RCC_BASE_ADDR) -> RCC_CFGR &= ~(1 << 12); //APB Low speed clock not divided
	((RCC_Reg_struct *) RCC_BASE_ADDR) -> RCC_CFGR &= ~(1 << 15); //APB High speed clock not divided
  ...
  /* USER CODE BEGIN 2 */
	((GPIO_Reg_struct *) GPIOD_BASE_ADDR) -> GPIOx_MODER &= ~(3 << 26); //Set GPIO mode to General purpose output
	((GPIO_Reg_struct *) GPIOD_BASE_ADDR) -> GPIOx_MODER |= (1 << 26); //Set GPIO mode to General purpose output
	((GPIO_Reg_struct *) GPIOD_BASE_ADDR) -> GPIOx_OTYPER &= ~(1 << 13); //Set no pullup/down
  ...
  while (1)
  {
    ((GPIO_Reg_struct *) GPIOD_BASE_ADDR) -> GPIOx_BSRR &= ~(1 << 29);
    ((GPIO_Reg_struct *) GPIOD_BASE_ADDR) -> GPIOx_BSRR |= (1 << 13);
    for(long int i = 0; i<1000000;i++) (void)i;
	((GPIO_Reg_struct *) GPIOD_BASE_ADDR) -> GPIOx_BSRR &= ~(1 << 13);
	((GPIO_Reg_struct *) GPIOD_BASE_ADDR) -> GPIOx_BSRR |= (1 << 29);
    for(long int i = 0; i<1000000;i++) (void)i;
